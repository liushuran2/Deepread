...
if __name__ == '__main__':
    ...
    parser.add_argument('--suns_model_path', default='suns/Model_CV10.h5', help='path to the trained model')
    ...
    parser.add_argument('--frames_init', type=int, default=50, help='buffer size of initialization')
    ...
    with tf.device('/GPU:0'):
        fff = get_shallow_unet()
    fff.load_weights(args_eval.suns_model_path)
    ...
    for directory_id in range(1):
        ...
        directory_path = '/mnt/nas03/LSR/DATA/2p_bench/2p_148d/trial_2p_' + str(directory_id) + '/'
        ...
        with torch.no_grad():
            for fnames in tiff_files:
                ...
                bb_all = video_raw
                timepoints, h, w = video_raw.shape
                # bb = video_raw
                template = torch.median(bb_all, dim=0, keepdim=False)[0]
                ...
                useSF=False, useTF=False, useSNR=True, med_subtract=False, \
                batch_size_init=100, useWT=True, p=p)
                ...
                template = template.view(1, 1, h, w).detach().cpu().numpy()
                ...
                for i in range(steps):
                    ...
                    # update template
                    # template = np.median(data_pr_middle, axis=0)
                    # template = torch.from_numpy(template).unsqueeze(0).unsqueeze(0)
                    ...
                    frames_SNR = preprocess_online_batch(bb, dimsnb, dimsnb, med_frame3, frame_SNR, \
                        None, None, None, None, \
                        None, useSF=False, useTF=False, useSNR=True, \
