...
from suns.PreProcessing.par1 import fastexp, fastmask, fastlog, \
    fastconv, fastquant, fastnormf, fastnormback, fastmediansubtract,fastnormf2
...
def preprocess_online_batch(bb, dimspad, dimsnb, med_frame3, frame_SNR=None, past_frames = None, \
        mask2=None, bf=None, fft_object_b=None, fft_object_c=None, Poisson_filt=np.array([1]), \
        useSF=True, useTF=True, useSNR=True, med_subtract=False, update_baseline=False):
    '''Pre-process the registered image in "bb" into an SNR image "frame_SNR" 
        using known median and median-based std in "med_frame3".
    ...
    if useSNR:
        # fastnormf_2(frame_SNR[:rowspad, :colspad], med_frame3[:, :rowspad, :colspad])
        fastnormf2(frame_SNR[:, :rowspad, :colspad], med_frame3[:, :rowspad, :colspad])
    else:
        # fastnormback_2(frame_SNR[:rowspad, :colspad], max(1, med_frame3[0,:colspad,:colspad].mean()))
        fastnormback(frame_SNR[:rowspad, :colspad], max(1, med_frame3[0,:colspad,:colspad].mean()))
...
def CNN_online(frame_SNR, fff, dims=None):
    ...
    frame_prob = fff.predict(frame_SNR, batch_size=1,verbose=0)
    frame_prob = frame_prob.squeeze()[:dims[0], :dims[1]] 
    return frame_prob

def CNN_online_batch(frame_SNR, fff, batch_size):
    '''Use CNN model "fff" to infer the probability of each pixel being active for image "frame_SNR".
    ...
    '''
    dims = frame_SNR.shape
    frame_SNR = frame_SNR[:,np.newaxis,:,:] 
    device = next(fff.parameters()).device
    frame_SNR = torch.from_numpy(frame_SNR).to(device)
    # network_input = frame_SNR.to(device)
    frame_prob = fff(frame_SNR)
    # frame_prob = fff.predict(frame_SNR, batch_size=batch_size,verbose=0)
    frame_prob = frame_prob.squeeze()[:dims[0], :dims[1], :dims[2]].detach().cpu().numpy() 
    return frame_prob
...
def separate_neuron_online_batch(frame_prob, pmaps_b, thresh_pmap_float, minArea, avgArea, useWT=False, useMP=True, p=None):
    '''Segment the probability map "frame_prob" into connected regions representing neurons.
        It disgards the regions whose areas are smaller than "minArea".
        When useWT=True, it further tries to segment neurons whose areas are larger than "avgArea" using watershed. 
        The outputs are the segmented masks and some statistics (areas, centers, and whether they are from watershed)
    ...
    '''
    # threshold the probability map to binary activity
    fastthreshold(frame_prob, pmaps_b, thresh_pmap_float)
    # spatial clustering each frame to form connected regions representing active neurons
    # segs = separate_neuron(pmaps_b, None, minArea, avgArea, useWT)
    if useMP:
        if p is None:
...
def separate_neuron_online_batch_Autothreshold(frame_prob, pmaps_b, thresh_pmap_float, minArea, avgArea, useWT=False, useMP=True, p=None):
    ...
    '''
    # threshold the probability map to binary activity
    thresh_pmap = np.float32(np.squeeze(np.percentile(frame_prob, 95, axis=(1,2))))
    exp_idx = np.exp2(np.mean(frame_prob, axis=(1,2)))
    thresh_pmap = thresh_pmap * exp_idx
    # thresh_pmap = np.clip(thresh_pmap, 1.0, exp_idx)
    fastthreshold_auto(frame_prob, pmaps_b, thresh_pmap)
    # spatial clustering each frame to form connected regions representing active neurons
    # segs = separate_neuron(pmaps_b, None, minArea, avgArea, useWT)
    if useMP:
        if p is None:
...
def extrace_trace(tuple_temp, prob_map, trace, frame_start=0):
    Masks = tuple_temp[0]
    ...
    for i in range(N):
        ...
        mask = np.reshape(Masks[i].toarray(), (Lx, Ly))
        ...
        trace[i, frame_start:frame_start+nframes] = curr_temporal_signal
