...
if __name__ == '__main__':
    ...
    parser.add_argument('--frames_init', type=int, default=300, help='buffer size of initialization')
    parser.add_argument('--batch_size', type=int, default=10, help='batch size of denoising')
    parser.add_argument('--overlap_size', type=int, default=4, help='batch size of denoising')
    ...
    root_path = '/mnt/nas02/LSR/DATA/ABO2024/'
    ...
    directories = ['506278598', '506540916', '569645690', '569722788', '577313742', '582838758', '584944065', '585953317', '604529230', '605035620', '627823695', '629789161', '639117180']
    ...
    for directory_id in directories:
        ...
        save_path = directory_path + 'DeepIE_50mW_scale10_stack16_fton10mW_170/'
        p = mp.Pool()
        ...
        with torch.no_grad():
            for fnames in tiff_files:
                ...
                video_adjust = test_batch(model, args_eval, bb, session_name, args_eval.data_property, template, batch_size,overlap_size,
                ...
                for i in range(steps):
                    ...
                    flow_pr, data_pr = model(template, image, iters=iters, timepoints=timepoints, flow_init=None, test_mode=True, denoise=True) # note flow_prev is only used for warm start
                    ...
                    if args_eval.doublestage:
                        ...
                        # update the template
                        # new_template = torch.median(data_fisrt, dim=1, keepdim=True)[0].expand(-1, timepoints, -1, -1, -1).contiguous()
                        # new_template = template.view(batchsize * timepoints, c, h, w)

                        # template = new_template

                        data_fisrt = data_fisrt.view(batchsize * timepoints, c, h, w)
                        flow_pr, data_pr = model(template, data_fisrt, iters=iters, timepoints=timepoints, flow_init=None, test_mode=True, denoise=False)
                    if i == 0:
                        ...
                        data_pr_middle = data_pr[0,0, overlap_size:overlap_size+batch_size].detach().cpu().numpy()
